...
def main():
    ...
    parser_train.add_argument('-dataset_path', help=path_constant, metavar="./data/factcheck.csv", required=True, type=str)
    ...
    parser_validate.add_argument('-dataset_path', help=path_constant, metavar="./data/factcheck.csv", type=str, required=True)
    ...
    try:
        if args.command == 'train':
            ...
            model = app.train_model(dataset_path=args.dataset_path, x=args.x, y=args.y, kfold=int(args.kfold),
                                    test_size=float(args.size), save=True if args.save else False, model=args.model,
                                    vectorizer=args.vectorizer)
            import joblib
            if model is not None:
                joblib.dump(model, args.save)
                print(f"Model saved in file: {args.save}")
            ...
            print(f"Visualization saved to file: {args.save}")
